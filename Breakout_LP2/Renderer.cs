...
namespace Breakout_LP2 {
    class Renderer : IRenderer {
        ...
        public void Setup(IBuffer2D<IGameobject> worldToRender) {
            // Limpar a consola
            Console.Clear();
        ...
        public void Render(IBuffer2D<IGameobject> worldToRender) {
            ...
            for (int y = 0; y < worldToRender.YDim; y++) {
                for (int x = 0; x < worldToRender.XDim; x++) {
                    if (worldToRender[x, y] != null) {
                        sb.Append(worldToRender[x, y].Render());
                    } else {
                        sb.Append(" ");
                    }
                }
